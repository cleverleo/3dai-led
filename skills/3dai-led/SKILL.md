---
name: 3dai-led
description: 3dai-led 硬件状态灯 — ESP32 + WS2812 灯带,HTTP 控制 8 颗灯珠实时反映 AI 工作状态。按会话抢占灯珠槽位,支持多目录 / 多会话 / 多工具并行。用于查询灯的状态、调试灯不亮、接入编码工具、调整灯珠分配或亮度。
---
...
- **`led.sh`**(+ 同目录的 `lease.py`)— 槽位租约脚本,解决多个工具 / 多个会话并行时争抢灯珠的问题
...
`skills/3dai-led/led.sh` 用租约机制解决:**按 (平台, 会话) 动态抢占一颗空闲灯珠,会话结束归还**。脚本靠自身路径找同目录的 `lease.py`,所以放哪、怎么调用(绝对路径、软链接、加进 `PATH`)都行。

### 命令

```bash
led.sh <state> [platform]   # 抢占或复用当前会话的槽位并点灯
led.sh release [platform]   # 归还当前会话的租约,熄灯并释放槽位
...
$ led.sh status
灯珠  归属
  0   [claude] /Users/me/work/project-a  (a1bb76fd, 3 秒前活动)
  1   [codex ] /Users/me/work/project-a  (0f21c8ab, 41 秒前活动)
  2   [claude] /Users/me/work/project-b  (7d5e0c14, 12 秒前活动)
  3   —
```

括号里是 `session_id` 的前 8 位。一个会话一颗灯珠,所以同一个目录出现在好几行里是正常的 —— 那是同时开着的几个会话。路径只是给人看的标签,不参与归属判定;会话中途换了工作目录(进 worktree、`cd` 到子目录),这一列最多滞后 `LED_TS_THROTTLE` 秒才跟上,灯珠本身不动。

### 为什么按会话而不是按工作目录

早先归属粒度是 (平台, 工作目录),换掉是因为「一个会话只待在一个目录里」这个前提根本不成立:Claude Code 进 git worktree、Bash 里 `cd` 到子目录,hook 进程的 `$PWD` 就变了,同一个会话会拿新 cwd 再抢一颗灯珠。

归还的问题更严重:`release` 只删当前 `$PWD` 算出的那一条,而会话退出时 `$PWD` 只可能落在其中一个目录上,其余几条永远等不到 `release`,只能熬满 `LED_STALE_MIN`。实际见过一个会话吃掉三颗灯珠、其中两颗冻在半路的状态上再也不动。

改成按会话之后:

- 一个会话一颗灯珠,从第一次点灯到退出都是同一颗,中途换目录不换灯
- `release` 就是删掉自己那条 key,不再需要引用计数,也不会漏
- 被 `SIGKILL` 的会话不再拖累别人 —— 它自己那条租约过期被回收就完了

代价是 8 颗灯珠消耗得更快:以前同一目录开三个会话共用一颗,现在是三颗。满槽后新会话静默不点灯(见下)。

### 平台:同一目录里并行的不同工具

`session_id` 只保证在自己那个工具里唯一,跨工具不保证不撞,所以平台名也要进 key。归属粒度是 **(平台, 会话)**,平台名按以下优先级取:
...
从按目录归属的旧版升级时,老租约(裸目录名的 key、和 `<平台>:<目录>` 的 key)没法映射到某个 `session_id` —— 一条目录租约背后可能压着好几个会话。所以不迁移,直接当过期条目回收:**升级后所有灯会灭一次**,活着的会话下一次 hook 就重新抢一颗。只闪这一下。

### 会话标识的两种来源

归属主键就是这个标识,脚本按以下优先级取:

1. **stdin 的 JSON** — 读 `session_id` 字段(工具以 hook 形式喂入时用这个)
2. **`LED_SESSION_ID` 环境变量** — 不走 stdin 的调用方用这个
3. 都没有则**退回按工作目录归属**,同目录的调用共用一颗灯珠(`status` 里这一行显示成「按目录」)

第 3 条不用一个固定的假 id 兜底是有原因的:固定值会让某个工具的所有会话挤在同一颗灯珠上,而且谁先结束谁就替所有人把灯熄了。退回目录至少保住按目录区分 —— 终端里手动敲 `led.sh` 排查、或者万一哪个工具不报 `session_id`,都靠这条。
...
{"claude:<session_id>": {"slot": 0, "ts": 1784962322, "cwd": "/Users/me/work/project-a"}}
```

- **归属按 (平台, 会话)** — key 唯一对应一个会话,`release` 就是删掉这条 key,不需要引用计数;不同平台是两条独立租约
- **`cwd` 是纯展示字段** — 只给 `status` 用,不参与任何判定。写路径上顺手刷新,所以换目录后最多滞后 `LED_TS_THROTTLE` 秒
- **没有锁,稳定态只读** — 会话已持有灯珠、`ts` 距今不到 `LED_TS_THROTTLE` 秒时,直接用记录里的灯珠号,一个字节都不写。一轮对话十几次 hook,只有第一次会写文件
- **写入原子** — 先写同目录临时文件再 `os.replace()`,读者要么看到旧的完整 JSON、要么看到新的,绝不会读到半截。两个进程恰好撞进同一个读-改-写窗口时,后写的会覆盖前者:最坏是两个会话映射到同一颗灯(状态互相覆盖),或某个条目丢失(下次点灯重新抢占,自愈)。对一个状态指示灯来说,这比维护一把锁划算 —— 锁会把 `curl` 的 2 秒超时算进持锁时间,设备离线时反过来让所有会话的灯集体不亮
...
数的是租约条数而不是目录数 —— 同一目录里并行的两个会话也是两颗灯,同样要切到多项目模式。
...
- **单项目模式强制归到 0 号灯珠** —— 因为该模式下 `led` 参数被忽略、状态落在 `led_states[0]`。如果唯一那条租约持有的是 3 号,切回多项目模式时 0 号会亮着一个没有主人的状态,而真正的主人在 3 号上却是灭的。所以退回单项目时会把它迁到 0 号,并熄掉旧槽位
- **顺序是「先熄灯 → 再切模式 → 再点灯」** —— 熄灯必须发生在切模式之前(那时 `led` 参数还有效,不会打到全局),点灯必须紧跟切模式(否则单项目模式下整条灯带会空着)

`release` 时**不**切模式。此刻切到单项目模式,整条灯带会去表现"最后一次 `set`"的值 —— 也就是刚发出去的那个 `off`,剩下那个会话的灯会莫名熄灭。留给它下次点灯时切,那时切模式和点亮状态在同一次调用里,不留空档。代价是从多项目退回单项目最多迟 `LED_TS_THROTTLE` 秒生效,期间维持原样。

`ts` 的节流顺带定了回收时机:活跃会话每 `LED_TS_THROTTLE` 秒必然走一次写路径,回收就挂在写路径上,不必每次点灯都扫一遍。所以别人残留的灯珠最迟在 `LED_STALE_MIN` 到期后再过 `LED_TS_THROTTLE` 秒被熄掉。

被 `SIGKILL` 的会话没机会 `release`,租约会一直占着灯珠;此后 `ts` 不再刷新,整条租约会在 `LED_STALE_MIN` 之后被回收并熄灯。这是唯一的泄漏路径,而且只影响它自己那颗灯。
...
不区分的话,按一次 `/clear` 灯会灭一下再亮,且**可能换灯珠** —— 空档期里别的会话可能抢先占了原槽位。
...
- **这一秒里在同一目录重开 opencode 也不会误熄。** 租约按会话归属,`release` 删的是自己那条 key,新会话是另一条,碰不着
- **不用信号处理器**(`process.on("SIGINT")` 之类)是刻意的:在 Node/Bun 里挂上监听会顶掉默认的终止行为,弄不好把 opencode 变成 Ctrl-C 杀不掉的进程。宁可多一个 `sh`

正常收到 `session.deleted` 而归还时,插件会先把对应的守护进程 kill 掉再还 —— 留着的话它会在退出时再还一次,虽然无害(`lease.py` 对已经不存在的 key 直接返回空),但 `debug.log` 里会多出一行看不懂的 `release`。守护进程还的那一次,`event=` 列记的是 `guard`,一眼能和插件自己发的区分开。
...
- `led=— s=— rc=—` 是占位行,表示这次一个 HTTP 请求都没发 —— `reason=clear`/`resume` 保住租约、和满槽降级都是这个形状
