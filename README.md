...
所以 8 颗灯珠按 **(平台, 会话)** 动态分配:每个会话抢占一颗,从第一次点灯到退出都是同一颗,中途换工作目录(进 git worktree、`cd` 到子目录)也不换灯;退出时归还熄灯。平台名一起进 key 是因为 `session_id` 跨工具不保证不撞 —— 有了它,同一个目录里并行的 Claude Code、codex、opencode 各占一颗灯,状态不互相覆盖,一方结束也不会顺手熄掉另一方的灯。只有一条租约在跑时自动切成整条灯带表现同一个状态(比只亮一颗醒目得多),第二条进来时自动切回独立模式。
