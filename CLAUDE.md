...
`data/leases.json` 形如 `{"<platform>:<session_id>": {"slot": 0, "ts": <epoch>, "cwd": "<仅展示>"}}`。归属粒度是 **(平台, 会话)** —— 一个会话一颗灯珠,从第一次点灯到退出都是同一颗,`release` 就是删掉自己那条 key。

**`cwd` 不参与归属判定**,只是 `status` 里给人看的标签,写路径上顺手刷新(所以换目录后最多滞后 `THROTTLE` 秒)。早先归属粒度是 (平台, 工作目录),换掉是因为「一个会话只待在一个目录里」不成立:进 worktree、Bash 里 `cd` 到子目录,hook 进程的 `$PWD` 就变了,同一个会话会拿新 cwd 再抢一颗灯珠;而 `release` 只删当前 `$PWD` 那一条,其余几条永远等不到归还,只能熬满 `STALE`。实测见过一个会话吃三颗灯珠、两颗冻在半路的状态上。别往回改。

会话号取值链是 stdin JSON 的 `session_id` > `LED_SESSION_ID` > **退回 `cwd:<工作目录>`**。最后那级不能换成一个固定的假 id:固定值会让某个工具的所有会话挤在同一颗灯上,而且谁先结束谁替所有人熄灯。平台名取值链是 `led.sh <state> [platform]` 的第二个参数 > `LED_PLATFORM` > `cli`;平台必须进 key,因为 `session_id` 只在自己那个工具里唯一。写成 hook 命令的参数而非 settings.json 的 env,是为了在配置里一眼看出哪条 hook 是谁挂的(`hooks_config.py` 里的 `PLATFORM` 常量)。

按目录归属时代的老租约(裸 cwd key、`<平台>:<cwd>` key)由 `migrate()` 置 `ts=0`,交给写路径上的 `sweep()` 当过期条目删掉并熄灯 —— 不迁移是因为一条目录租约背后可能压着好几个会话,没法映射到某个 sid。升级时灯会集体灭一次,下一次 hook 各自重抢。

无锁是刻意的,推导写在 `lease.py` 顶部注释里 —— 动这块之前先读完。三条不变量:

1. **稳定态零写入**。会话已持有灯珠 + `ts` 距今 < `THROTTLE` 时走快路径,直接返回灯珠号,不写文件、不发 `/mode`。一轮对话十几次 hook 只有第一次落盘。
2. **写入靠 `os.replace()` 原子替换**,丢更新可接受(最坏是两个会话撞同一颗灯,或条目丢失后下次自愈)。不要为它引入锁 —— 锁会把 curl 的 2 秒超时算进持锁时间,设备离线时反而让所有会话的灯集体不亮。
3. **过期回收挂在写路径上**。活跃会话每 `THROTTLE` 秒必然写一次,`sweep()` 顺手清掉 `ts` 超过 `STALE` 的残留,不必每次点灯都扫。
...
- **`target_mode()` 数的是租约条数,不是目录数** —— 同一目录里两个会话并行也必须切多项目模式。
- **`:` 是租约 key 的分隔符**,平台名里出现会被 `norm_platform()` 换成 `-`;`partition(":")` 只切第一个,所以 sid 里含 `:` 无害。判定旧格式 key 靠的是「`:` 后面那截以 `/` 开头」—— sid 不会以 `/` 开头。
